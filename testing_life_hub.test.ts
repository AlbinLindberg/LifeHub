...
import { ActivityTable, string_to_number , Activity, makeData} from "./LifeHub_albin";
...
let credentials: { [key: string]: string } = {}
    ...
    function makeData_changed(month: string, date: string, start: string, end: string, activity: string): void {
        ...
        const data: ActivityTable = ph_empty(10, probe_linear(hashfunc));// for testing sake we just create a empty hashtable 
...
test("make some data", () => {
    ...
    expect(calendar_data.data).toContain({month: "januari", date: "23", start: "13:00", end: "15:00", activity: "Fotboll"})
})
/*
test("add a activity", () => {
        ...
        expect(calendar_data.data).toContain({month: "january", date: "23", start: "13:00", end: "14:00", activity: "Football"});
...
test("remove activity"), () => {
    add_changed("january", "25", "13:00", "14:00", "Handball");
        const calendar_data: ActivityTable = mock_storage[global_username + '_data']; // accesing the correct users data
        expect(calendar_data.data).toContain({month: "january", date: "25", start: "13:00", end: "14:00", activity: "Handball"});
...
test("search for activity "), () => {
    ...
    expect(searchActivity_changed("januari", "27")).toBe({month: "januari", date: "27", start: "13:00", end: "14:00", activity: "Icehockey"});
}*/