...
const manipulate = (): void => {
    ...
    for (let i = 1; i <= lastdate; i++) {
        ...
        lit += `<li class="${isToday}" onclick="searchOnDate(${year}, ${month}, ${i})">${i}</li>`
    }
...
prenexIcons.forEach(icon => {
    ...
    function handleActionKeyPress2(event: KeyboardEvent): void {
        if (event.key === "Enter") {
            ...
            const startElement: HTMLElement | null = document.getElementById("login");
            ...
            //(document.getElementById("kalender") as HTMLElement).style.display = "none"
                if (action === "l" && loginFormElement && createFormElement && startElement) {
    ...
    function makeData(month: string, date: string, start: string, end: string, activity: string): void {
        ...
        const data: ActivityTable = data_con ? JSON.parse(data_con) : ph_empty(100, probe_linear(hashfunc));
    ...
    function add(): void {
        const modalElement: HTMLInputElement | null = document.getElementById("myModal") as HTMLInputElement
        const kalender: HTMLInputElement | null = document.getElementById("kalender") as HTMLInputElement;
        const login: HTMLInputElement | null = document.getElementById("login") as HTMLInputElement;
        kalender.style.display = "none";
        login.style.display = "none";
    ...
    function searchOnDate(year: number, month: number, date: number): void {
        const searchMonth: string | undefined = months[month]; 
        const searchDate: string = date.toString()
        const messageElement: HTMLElement | null = document.getElementById("message");
    
        if (searchMonth && searchDate && messageElement) {
            const activities: Activity[] = searchActivity(searchMonth, searchDate);
    
            if (activities.length > 0) {
                let message: string = `Activities found for ${searchMonth} ${searchDate}:\n`;
                activities.forEach(activity => {
                    message += `Start: ${activity.Start}, End: ${activity.End}, Activity: ${activity.Activity}\n`;
                });
                messageElement.innerText = message;
    ...
    function is_today(){
        ...
        const startElement: HTMLElement | null = document.getElementById("login");
        if (month && date && start && end && activity && modalElement && startElement) {
            const month_value: string = month.value;
            const date_value: string = date.value;
            const start_value: string = start.value;
            const end_value: string = end.value; 
            ...
            showMessage("added activities on chosen dates");
    }}
    ...
    /*function is_week(){
        ...
        const månader_array:Array<string> = [
            "January",
            "February",
            "Mars",
            "April", 
    ...
    function is_month(){
        ...
        makeData(månader_array[i], date_value, start_value, end_value, activity_value);}
        modalElement.style.display = "block";
        startElement.style.display = "none";
        showMessage("added activities on chosen dates");}
    }
